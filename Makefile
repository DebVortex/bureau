...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) bureau/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) bureau/manage.py$(1)
endif
endef
...
create-db:
	envdir envs/$(ENV) createdb -U bureau -l en_US.utf-8 -E utf-8 -O bureau -T template0 -e bureau

create-db-user:
	psql -d postgres -c "CREATE USER \"bureau\" WITH PASSWORD 'bureau' CREATEDB;"

drop-db:
	envdir envs/$(ENV) dropdb -i -e -U bureau bureau
...
isort:
	isort --recursive setup.py bureau/ tests/
...
startapp:
	...
	mkdir -p bureau/apps/$$app_name; \
	envdir envs/$(ENV) python manage.py startapp $$app_name bureau/apps/$$app_name --template bureau/config/app_template; \
	echo "Don't forget to add 'bureau.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'bureau.config/settings/common.py'!"
...
test-upload:
	...
	python -c "import webbrowser; webbrowser.open('https://testpypi.python.org/pypi/bureau')"

upload:
	twine upload -s dist/*
	python -c "import webbrowser; webbrowser.open('https://pypi.python.org/pypi/bureau')"
